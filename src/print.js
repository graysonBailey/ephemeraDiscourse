...
const state = new p5((p) => {
  ...
  p.setup = function() {
    cnv = p.createCanvas(plotWidth, 10000)
    p.textFont(tFont)
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < origin.length; i++) {
        ...
        tempDoc.position(origin[i].p.x-200, origin[i].p.y)
      ...
      for (let each in origin) {
        if (origin[each].r.length > 0) {
          ...
          if(externals.length>0){
            ...
            if(origin[each].p.x >550){
              p.text("e x t e r n a l :", origin[each].p.x-200, origin[each].p.y+40)
      ...
      for (let i = 0; i < origin.length; i++) {
        ...
        let elem = p.createSpan(spl[0]).class('discourseElement')

        if (spl[0].charAt(0) == 'r' && spl[0].charAt(1) == '/') {
          elem.addClass('response')
        } else if (spl[0].charAt(0) == 'c' && spl[0].charAt(1) == '/') {
          elem.addClass('composition')
        } else if (spl[0].charAt(0) == 'q' && spl[0].charAt(1) == '/') {
          elem.addClass('quote')
        } else if (spl[0].charAt(0) == 'q' && spl[0].charAt(1) == '/'){
          elem.addClass('editorial')
        }

        elem.id = origin[i].u
        elem.position(origin[i].p.x, origin[i].p.y)
        elem.attribute('contenteditable', true)

        let quickHeight = elem.size().height

        let cite = p.createSpan(spl[1]).class('discourseCitation')
        cite.id = "cite" + origin[i].u
        cite.position(origin[i].p.x, origin[i].p.y + quickHeight+5)
        cite.attribute('contenteditable', true)

        let above = p.createSpan(origin[i].u + "  : (" + origin[i].d + ")").class('discourseCitation')
        above.position(origin[i].p.x, origin[i].p.y - 18)
      }
